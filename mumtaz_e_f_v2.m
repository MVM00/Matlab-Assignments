...
% plotting these five complex numbers as vectors 
A = [z1;z2;z3;z4;z5]; % with compass
figure 
compass(real(A),imag(A)); % with quiver 
quiver(zeros(size(A,1),1),zeros(size(A,1),1),real(A),imag(A));% with quiver

figure (1)
title('Five complex numbers as vector ');
xlabel('Time (s)');
ylabel('Amplitude (m)');
